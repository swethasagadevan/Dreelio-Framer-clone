...
const Blog = () => {
  ...
  return (
    <div id="blog" className="container mt-16 p-4 max-w-6xl mx-auto">
      <motion.div
      ...
      >
        <span className="text-xs text-gray-500 font-bold bg-white rounded-full p-2">
          BLOG
        </span>
      ...
      >
        ...
        >
          ...
          <div className="py-8 ">
            ...
            <h4 className="font-semibold text-2xl md:text-3xl xl:text-4xl mt-7">
      ...
      <motion.div
        ...
        whileInView="show"
        viewport={{ once: true }}
        className="mt-10 xl:grid grid-cols-3 gap-5 p-4"
      >
        {blogFratures.map((data, i) => (
          <motion.div variants={itemVariants} key={i}>
            ...
            <div className=" flex justify-between items-center py-2 pr-1">
              ...
              <div>
                <button
                  className={`text-white text-xs font-bold rounded-full py-1 px-3  ${i == 0 ? "bg-blue-600" : i == 1 ? "bg-yellow-600" : "bg-green-600"}`}
                >
                  {data.btntxt}
                </button>
              </div>
